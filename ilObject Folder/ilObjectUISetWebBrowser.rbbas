#tag Class
Protected Class ilObjectUISetWebBrowser
Inherits ilObjectBase
	#tag Event
		Sub GenerateAndroidCode(theOrigin as string)
		  ...
		  if tempILUI <> nil then
		    ...
		    s = s + tempILUI.ISCClassLocalAndroidName + " = (WebView)this.findViewById(R.id." + tempILUI.ISCClassLocalAndroidName +");" + EndOfLine
		    s = s + tempILUI.ISCClassLocalAndroidName + ".setWebViewClient(new ISCWebViewClient());" + EndOfLine
		    s = s + tempILUI.ISCClassLocalAndroidName + ".getSettings().setJavaScriptEnabled(true);" + EndOfLine
		    
		    s = s + tempILUI.ISCClassLocalAndroidName + ".loadUrl(" + modVariables.VariableObjectFromName(MessageFromVariable).ISCClassLocalAndroidName + ");" + EndOfLine
	...
	#tag Event
		Sub GenerateiOSCode(theOrigin as string)
		  ...
		  if tempILUI <> nil then
		    ...
		    s = s + "[" + tempILUI.ISCiOSName + " loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:" + _
		     modVariables.VariableObjectFromName(MessageFromVariable).ISCiOSName + "]]];" + EndOfLine
		    
		    //[myWeb loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://www.lunduke.com/"]]];
	...
	#tag Event
		Sub GeneratePythonDesktopCode(theOrigin as string)
		  ...
		  if tempILUI <> nil then
		    ...
		    s = s + " " + tempILUI.ISCPythonName + ".set_text(" + modVariables.VariableObjectFromName(MessageFromVariable).ISCPythonName + ")" + EndOfLine
	...
	#tag Method, Flags = &h1000
		Sub Constructor()
		  ...
		  me.CodeNameBase = "iscSetWebBrowser"
		  me.BGColor =  frmProject.kcolUserInterface
		  me.DisplayName = "Set WebBrowser"
		  me.DisplayCategory = "User Interface"
		  me.Title = "Set WebBrowser"
		  me.Width = 90
