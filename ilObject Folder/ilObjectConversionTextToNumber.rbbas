#tag Class
Protected Class ilObjectConversionTextToNumber
Inherits ilObjectBase
	#tag Event
		Sub GenerateAndroidCode(theOrigin as string)
		  ...
		  s = s + modVariables.VariableObjectFromName(MessageFromVariable).ISCClassLocalAndroidName + " = Integer.parseInt("
		  
		  s = s + modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalAndroidName + ");" + EndOfLine
		  
		  s = s + "//" + GetCodeName + "Done" + EndOfLine
		  
		  s = s + "}" + EndOfLine + EndOfLine
		  
		  setCodeToCallThisObjectForName("Convert", GetCodeName + "();")
	...
	#tag Event
		Sub GenerateFlexWebCode(theOrigin as string)
		  ...
		  "parseInt(" + modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalFlexName + ");" + EndOfLine
	...
	#tag Event
		Sub GenerateiOSCode(theOrigin as string)
		  ...
		  s = s + modVariables.VariableObjectFromName(MessageFromVariable).ISCiOSName + " = "
		  s = s + "[" + modVariables.VariableObjectFromName(OriginVariable).ISCiOSName + " intValue];" + EndOfLine
		  
		  s = s + "//" + GetCodeName + "Done" + EndOfLine
		  s = s + "" + EndOfLine
		  s = s + "}" + EndOfLine + EndOfLine
		  
		  setCodeToCallThisObjectForName("Convert", "[self " + GetCodeName + "];")
	...
	#tag Event
		Sub GenerateJavaScriptWebCode(theOrigin as string)
		  ...
		  s = s + modVariables.VariableObjectFromName(MessageFromVariable).ISCClassLocalFlexName + " = parseInt("
		  
		  s = s + modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalFlexName + ");" + EndOfLine
	...
	#tag Event
		Sub GeneratePythonDesktopCode(theOrigin as string)
		  ...
		  s = s + " try:" + EndOfLine
		  s = s + "  " + modVariables.VariableObjectFromName(MessageFromVariable).ISCPythonName + " = " + _
		  "int(" + modVariables.VariableObjectFromName(OriginVariable).ISCPythonName + ")" + EndOfLine
		  
		  s = s + " except ValueError:" + EndOfLine + _
		  "  " + modVariables.VariableObjectFromName(MessageFromVariable).ISCPythonName + " = 0" + EndOfLine
	...
	#tag Method, Flags = &h1000
		Sub Constructor()
		  ...
		  me.CodeNameBase = "iscConvertTextToNumber"
		  me.BGColor =  frmProject.kcolConversion
		  me.DisplayName = "Convert Text to Number"
		  me.DisplayCategory = "Conversion"
		  me.Title = "Convert Text to Number"
		  me.Width = 110
		  
		  me.ObjectItems.Append "Convert"
