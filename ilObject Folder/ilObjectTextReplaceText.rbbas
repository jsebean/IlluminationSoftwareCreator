#tag Class
Protected Class ilObjectTextReplaceText
Inherits ilObjectBase
	#tag Event
		Sub GenerateAndroidCode(theOrigin as string)
		  ...
		  modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalAndroidName + ".replaceAll(" + _
		  ...
		  'string.split("a").join("b");
		  
		  s = s + "//" + GetCodeName + "Done" + EndOfLine
		  
		  s = s + "}" + EndOfLine + EndOfLine
		  
		  setCodeToCallThisObjectForName("Replace", GetCodeName + "();")
	...
	#tag Event
		Sub GenerateFlexWebCode(theOrigin as string)
		  ...
		  's = s + "var myRegExp:RegExp=new RegExp(" + modVariables.VariableObjectFromName(MathVariable1).ISCPythonName + ",""g"");" + EndOfLine
		  'str.replace(myRegExp, “xyz”);
		  
		  s = s + modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalFlexName +" = " + _
		  modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalFlexName + ".split(" + _
		  modVariables.VariableObjectFromName(MathVariable1).ISCClassLocalFlexName + ").join(" + _
	...
	#tag Event
		Sub GenerateiOSCode(theOrigin as string)
		  ...
		  s = s + "[" + modVariables.VariableObjectFromName(OriginVariable).ISCiOSName + " replaceOccurrencesOfString:" + _
		  modVariables.VariableObjectFromName(MathVariable1).ISCiOSName + " options:NSCaseInsensitiveSearch range:(NSRange){0,[" + _
		  modVariables.VariableObjectFromName(MathVariable2).ISCiOSName + " length]}];" + EndOfLine
		  
		  s = s + "//" + GetCodeName + "Done" + EndOfLine
		  s = s + "" + EndOfLine
		  s = s + "}" + EndOfLine + EndOfLine
		  
		  setCodeToCallThisObjectForName("Replace", "[self " + GetCodeName + "];")
	...
	#tag Event
		Sub GeneratePythonDesktopCode(theOrigin as string)
		  ...
		  modVariables.VariableObjectFromName(OriginVariable).ISCPythonName + ".replace(" + _
	...
	#tag Method, Flags = &h1000
		Sub Constructor()
		  ...
		  me.CodeNameBase = "iscReplaceText"
		  me.BGColor =  frmProject.kcolText
		  me.DisplayName = "ReplaceText"
		  me.DisplayCategory = "Text"
		  me.Title = "ReplaceText"
		  me.Width = 90
		  
		  me.ObjectItems.Append "Replace"
