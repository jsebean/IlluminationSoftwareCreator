#tag Class
Protected Class ilObjectUISetProgressBar
Inherits ilObjectBase
	#tag Event
		Sub GenerateAndroidCode(theOrigin as string)
		  ...
		  if tempILUI <> nil then
		    ...
		    s = s + tempILUI.ISCClassLocalAndroidName + " = (ProgressBar)this.findViewById(R.id." + tempILUI.ISCClassLocalAndroidName +");" + EndOfLine
		    
		    s = s + tempILUI.ISCClassLocalAndroidName + ".setMax(100);" + EndOfLine
		    s = s + tempILUI.ISCClassLocalAndroidName + ".setProgress(" + modVariables.VariableObjectFromName(MessageFromVariable).ISCClassLocalAndroidName + ");" + EndOfLine
	...
	#tag Event
		Sub GenerateFlexWebCode(theOrigin as string)
		  ...
		  if tempILUI <> nil then
		    ...
		    s = s + tempILUI.ISCClassLocalFlexName + ".setProgress(" + modVariables.VariableObjectFromName(MessageFromVariable).ISCClassLocalFlexName + ", 100);" + EndOfLine
	...
	#tag Event
		Sub GenerateiOSCode(theOrigin as string)
		  ...
		  if tempILUI <> nil then
		    ...
		    s = s + tempILUI.ISCClassLocalFlexName + ".progress = (" + modVariables.VariableObjectFromName(MessageFromVariable).ISCClassLocalFlexName + " / 100);" + EndOfLine
	...
	#tag Event
		Sub GenerateJavaScriptWebCode(theOrigin as string)
		  ...
		  s = s + "document.getElementById(""" + tempILUI.ISCClassLocalFlexName + """).value=" + _
	...
	#tag Event
		Sub GeneratePythonDesktopCode(theOrigin as string)
		  ...
		  if tempILUI <> nil then
		    ...
		    s = s + " " + tempILUI.ISCPythonName + ".set_fraction(float(" + modVariables.VariableObjectFromName(MessageFromVariable).ISCPythonName + ") / 100)" + EndOfLine
	...
	#tag Method, Flags = &h1000
		Sub Constructor()
		  ...
		  me.CodeNameBase = "iscSetProgressBar"
		  me.BGColor =  frmProject.kcolUserInterface
		  me.DisplayName = "Set ProgressBar"
		  me.DisplayCategory = "User Interface"
		  me.Title = "Set ProgressBar"
		  me.Width = 90
